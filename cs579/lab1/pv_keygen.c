...
{
  /* YOUR CODE HERE */
  size_t raw_sklen = 2 * CCA_STRENGTH;
  void *raw_sk = malloc(raw_sklen);
  ...
  if (argc != 2) {
    ...
    /* YOUR CODE HERE */
	prng_getbytes(raw_sk, raw_sklen);

    /* now let's armor and dump to disk the symmetric key buffer */

    /* YOUR CODE HERE */
	write_skfile(argv[1], raw_sk, raw_sklen);
