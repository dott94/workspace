...
private:
    ...
    AsciiNode* build_ascii_tree_recursive(Node<K, V> *node) {
        ...
        ascii_node->left = build_ascii_tree_recursive(node->left_);
        ascii_node->right = build_ascii_tree_recursive(node->right_);
        ...
        oss << node->kv_pair_.first;

        oss_second << node->kv_pair_.second;
        if (oss.str() != oss_second.str()) {
            oss << ":" << node->kv_pair_.second;
        }
